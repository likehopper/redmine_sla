...
(
    ...
    SELECT DISTINCT issues.id AS issue_id,
        ...
        COALESCE(first_value(journal_details.old_value::inte... window_journals, issues.status_id) AS journal_detail_old_value,
        COALESCE(first_value(journal_details.old_value::inte... OVER window_journals, issues.status_id) AS journal_detail_value
    FROM issues
    LEFT JOIN journals ON ( issues.id = journals.journalized_id )
    LEFT JOIN journal_details ON ( journals.id = journal_details...erty LIKE 'attr' AND journal_details.prop_key LIKE 'status_id' )
    ...
    INNER JOIN journal_details ON ( journals.id = journal_details...erty LIKE 'attr' AND journal_details.prop_key LIKE 'status_id' )
)