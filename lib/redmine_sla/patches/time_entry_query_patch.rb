...
# File: redmine_sla/lib/redmine_sla/patches/time_entry_query_patch.rb
# Purpose:
#   Extend Redmine's TimeEntryQuery class to provide SLA-related:
#     - filters (SLA level, SLA respect),
#     - dynamic SQL-based filter logic,
#     - additional columns for SLA level and SLA respect,
#     - dynamic methods per SLA type.
#
#   These additions allow the "Spent Time" list (time_entries#index)
#   to display and filter SLA information linked to each time entry's issue.
#
...
module RedmineSla
  module Patches
    ...
    # Patch applied dynamically to TimeEntryQuery
    module TimeEntryQueryPatch
      ...
      def self.included(base)
        ...
        base.class_eval do
          ...
          #
          # SQL condition builder for SLA-respect filters in time entries listing
          #
          ...
          #
          # SQL condition builder for SLA-level filter
          #
    ...
    # Instance methods injected into TimeEntryQuery
      ...
      def available_filters_with_sla_time_entry
        ...
        if @available_filters.blank? &&
          ...
          time_entry_get_sla_level = QueryColumn.new(
            ...
            groupable: true,
            sortable: "(SELECT sla_levels.name
                       FROM sla_caches
                       INNER JOIN sla_levels ON (sla_caches.sla_level_id = sla_levels.id)
                       WHERE sla_caches.issue_id = issues.id
                       ORDER BY sla_levels.name)"
          )
          ...
          #
          # SLA TYPES: dynamic columns and filters
          #
            ...
            SlaType
              ...
              time_entry_get_sla_respect = QueryColumn.new(
                ...
                sortable: "(
                    ...
                    LEFT JOIN custom_values ON (
                      sla_levels.custom_field_id = custom_values.custom_field_id
                      AND custom_values.customized_id = sla_issues.id
                    )
                    LEFT JOIN sla_cache_spents ON (
                      sla_caches.id = sla_cache_spents.sla_cache_id
                      AND sla_cache_spents.sla_type_id = #{sla_type.id}
                    )
                    LEFT JOIN sla_level_terms ON (
                      sla_caches.sla_level_id = sla_level_terms.sla_level_id
                      AND sla_level_terms.sla_type_id = #{sla_type.id}
                      AND sla_level_terms.sla_priority_id = (
                        CASE
          ...
          return available_filters_without_sla_time_entry
        end
