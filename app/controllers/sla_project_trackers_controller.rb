...
# File: redmine_sla/app/controllers/sla_project_trackers_controller.rb
# Purpose:
#   Manage the association between Projects, Trackers and SLAs (SlaProjectTracker).
#   This controller provides:
#     - listing via SlaProjectTrackerQuery,
#     - creation and update from project settings or global view,
#     - deletion and context menu support,
#     - API access for index/create/update/destroy.
#
...
class SlaProjectTrackersController < ApplicationController
  ...
  # List SlaProjectTrackers using SlaProjectTrackerQuery.
  # Can be filtered per project or globally.
  def index
    ...
    @entities     = @query.sla_project_trackers(
  ...
  # Render form for a new Project/Tracker/SLA association.
  def new
    @sla_project_tracker = SlaProjectTracker.new
  end

  # Create a SlaProjectTracker (optionally bound to a specific project).
  def create
    @sla_project_tracker = SlaProjectTracker.new
      ...
      respond_to do |format|
        format.html do
          ...
          redirect_back_or_default(
            @project.nil? ? sla_project_trackers_path : settings_project_path(@project, tab: :sla)
          )
        end
        format.api do
  ...
  # Update an existing SlaProjectTracker.
  def update
    @sla_project_tracker.safe_attributes = params[:sla_project_tracker]
    @sla_project_tracker.project = @project unless @project.nil?
    ...
    if @sla_project_tracker.save
      respond_to do |format|
        format.html do
          ...
          # Display a warning if there were any changes, since SLA configuration
          # can impact existing issues and caches.
  ...
  # Destroy one or more SlaProjectTrackers.
  def destroy
  ...
  # Context menu for bulk actions on SlaProjectTrackers.
  def context_menu
...
private
  ...
  # Load default SlaProjectTrackerQuery if none is explicitly chosen.
  ...
  # NOTE:
  # The following method is a duplicate definition of retrieve_default_query
  # kept as-is to avoid changing the original code behavior.
  def retrieve_default_query(use_session)
    return if params[:query_id].present?
    return if api_request?
    return if params[:set_filter]
