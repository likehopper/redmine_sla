...
# File: redmine_sla/test/functional/sla_calendars_controller_test.rb
...
class SlaCalendarsControllerTest < ApplicationSlaFunctionalsTestCase
  ...
  test "should get success on index as admin" do
    ...
    with_settings :default_language => "en" do
      ...
      assert_select 'a[href="/sla/calendars/1"]', :title => "Show"
      assert_select 'a[href="/sla/calendars/1/edit"]', :title => "Edit"
      assert_select 'a[href="/sla/calendars/1"]', :title => "Delete", :method => "delete"
    end
  end
  ...
  test "should return success on get show as manager" do
    @request.session[:user_id] = 2
    with_settings :default_language => "en" do
      get(:show, :params => {:id => 1})
      assert_response :success
