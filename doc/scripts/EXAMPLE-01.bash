...
#
STATUS_ID="1" # New
DATA="{ \"sla_status\": { \"sla_type_id\": \"$SLA_TYPE_ID\", \"status_id\": \"$STATUS_ID\" } }"
...
if [ $((ID+0)) -gt 0 ]; then
        ...
        if [ "$ERROR" == "SLA Type has already been taken" ]; then
                SEARCH="sla_type_id=$SLA_TYPE_ID&status_id=$STATUS_ID"
...
for i in $(echo ${!LEVEL_TERMS[@]}); do
  ...
  DATA="{ \"sla_level_term\": { \"sla_level_id\": \"$SLA_LEVEL_ID\", \"sla_type_id\": \"$SLA_TYPE_ID\", $LEVEL_TERM } }"
  ...
  if [ $((ID+0)) -gt 0 ]; then
          ...
          if [ "$ERROR" == "SLA Level This term alredy exists" ]; then
                  ...
                  SEARCH="sla_level_id=$SLA_LEVEL_ID&sla_type_id=$SLA_TYPE_ID&priority_id=$PRIORITY_ID"
